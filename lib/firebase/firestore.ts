...
export const getAllCreators = async (limitCount = 50): Promise<CreatorProfile[]> => {
  try {
    ...
    const q = query(usersRef, where("userType", "==", "creator"), limit(limitCount * 2))
    const querySnapshot = await getDocs(q)
    ...
    return creators.sort((a, b) => (b.followerCount || 0) - (a.followerCount || 0)).slice(0, limitCount)
  } catch (error) {
...
export const getAllBronzeHighlights = async (
  ...
  try {
    const highlightsRef = collection(db, "creator-highlights")
    const q = query(highlightsRef, where("requiredLevel", "==", "Bronze"), limit(limitCount))
    const querySnapshot = await getDocs(q)
...
// IMPROVED: Improved cache management to prevent stale expired stories
export const getCreatorActiveStories = async (creatorId: string): Promise<TemporaryStory[]> => {
  try {
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
      const validStories = cached.stories.filter((story) => {
        const expiresAt = story.expiresAt?.toDate ? story.expiresAt.toDate() : new Date(story.expiresAt)
        return expiresAt > now
      })

      // If we filtered out stories, update cache
      if (validStories.length !== cached.stories.length) {
        storiesCache.set(creatorId, { stories: validStories, timestamp: Date.now() })
      }
...
export const createTemporaryStory = async (
  ...
  try {
    ...
    console.error("Error creating temporary story:", error)
    throw error
  }
}
...
export const toggleStoryLike = async (userId: string, storyId: string, storyType: "temporary" | "highlight") => {
  try {
    const currentUser = auth.currentUser
    if (!currentUser) {
      throw new Error("Você precisa estar autenticado para curtir stories")
    }
    ...
    const storyCollection = storyType === "temporary" ? "temporaryStories" : "creator-highlights"
    const storyDoc = await getDoc(doc(db, storyCollection, storyId))
    if (!storyDoc.exists()) {
      throw new Error("Story não encontrado")
    }

    const likesRef = collection(db, "story-likes")
    const q = query(likesRef, where("userId", "==", userId), where("storyId", "==", storyId))
    const querySnapshot = await getDocs(q)
    ...
    if (querySnapshot.empty) {
      ...
      // Incrementar contador de likes no story
      await updateDoc(doc(db, storyCollection, storyId), {
        likes: increment(1),
      })

      const updatedStoryDoc = await getDoc(doc(db, storyCollection, storyId))
      const likeCount = updatedStoryDoc.data()?.likes || 0
    ...
    console.error("[v0] Error toggling story like:", error)
    throw error
  }
}
...
export const addStoryComment = async (
  ...
  try {
    ...
    const commentRef = await addDoc(collection(db, "story-comments"), {
      userId,
      storyId,
      storyType,
      text,
    ...
    // Incrementar contador de comentários no story
    await updateDoc(doc(db, storyCollection, storyId), {
      comments: increment(1),
    })
    ...
    console.error("[v0] Error adding story comment:", error)
    throw error
  }
}
...
export const getStoryComments = async (storyId: string) => {
  try {
    const commentsRef = collection(db, "story-comments")
    const q = query(commentsRef, where("storyId", "==", storyId), limit(50))
    const querySnapshot = await getDocs(q)
    ...
    console.error("[v0] Error getting story comments:", error)
    return []
  }
}

export const checkStoryLiked = async (userId: string, storyId: string): Promise<boolean> => {
  try {
    const likesRef = collection(db, "story-likes")
    const q = query(likesRef, where("userId", "==", userId), where("storyId", "==", storyId))
    const querySnapshot = await getDocs(q)
    ...
    console.error("[v0] Error checking story like:", error)
    return false
  }
}
