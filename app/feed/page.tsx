...
export default function FeedPage() {
  ...
  const { likedPosts, toggleLike: handleLikeToggle } = usePostLikes(user?.uid, posts)
  const { retweetedPosts, toggleRetweet: handleRetweetToggle } = usePostRetweets(user?.uid, posts)
        ...
        for (let i = 0; i < uniqueCreatorIds.length; i += batchSize) {
          ...
          const batchResults = await Promise.all(
            batch.map(async (creatorId) => {
              try {
                const [stories, hasUnviewed] = await Promise.all([
                  getCreatorActiveStories(creatorId),
                  hasUnviewedStories(creatorId, user.uid),
                ])
  ...
  if (loading) {
    return (
      <ResponsiveLayout showSuggestions={false} showMessages={false}>
  ...
  return (
    <ResponsiveLayout showMessages={true}>
      <div className="min-h-screen bg-background">
        ...
        {showNewPostsNotification && newPostsCount > 0 && (
          <div className="sticky top-14 sm:top-16 lg:top-0 z-40 bg-primary/10 border-b border-primary/20">
        ...
        <main className="pb-20 lg:pb-6 lg:max-w-2xl lg:mx-auto">
        ...
        {commentModalOpen && selectedPostId && (
          <CommentModal postId={selectedPostId} onClose={() => setCommentModalOpen(false)} />
        )}

        {tipModalOpen && selectedTipPost && <TipModal post={selectedTipPost} onClose={() => setTipModalOpen(false)} />}

        {isStoryViewerOpen && selectedCreatorStories && (
          <StoryViewer
