...
const PaywallModal = dynamic(
  () => import("@/components/paywall-modal").then((mod) => ({ default: mod.PaywallModal })),
  {
    loading: () => (
...
const TipModal = dynamic(() => import("@/components/tip-modal").then((mod) => ({ default: mod.TipModal })), {
  loading: () => (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
      <Loader2 className="h-8 w-8 animate-spin" />
    </div>
...
const CommentModal = dynamic(
  () => import("@/components/comment-modal").then((mod) => ({ default: mod.CommentModal })),
  {
    loading: () => (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
        <Loader2 className="h-8 w-8 animate-spin" />
...
const ShareProfileModal = dynamic(
  () => import("@/components/share-profile-modal").then((mod) => ({ default: mod.ShareProfileModal })),
  {
    loading: () => (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
        <Loader2 className="h-8 w-8 animate-spin" />
...
const ProfilePictureMenu = dynamic(
  () => import("@/components/profile-picture-menu").then((mod) => ({ default: mod.ProfilePictureMenu })),
  {
    ssr: false,
  },
)

const ProfilePictureViewer = dynamic(
  () => import("@/components/profile-picture-menu").then((mod) => ({ default: mod.ProfilePictureViewer })),
  {
    loading: () => (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
        <Loader2 className="h-8 w-8 animate-spin" />
...
// import { ProfilePictureMenu, ProfilePictureViewer } from "@/components/profile-picture-menu"

import { usePostLikes } from "@/hooks/use-post-likes"
import { usePostRetweets } from "@/hooks/use-post-retweets"
...
export default function CreatorProfilePage() {
  ...
  const { likedPosts, toggleLike: handleLikeToggle } = usePostLikes(user?.uid, displayedPosts)
  const { retweetedPosts, toggleRetweet: handleRetweetToggle } = usePostRetweets(user?.uid, displayedPosts)
  ...
  const [xpNotification, setXpNotification] = useState({ show: false, xpGained: 0, action: "" })
  const [paywallToast, setPaywallToast] = useState({
    isOpen: false,
    featureName: "",
    requiredTier: "" as "Gold" | "Platinum" | "Diamante" | undefined,
  })
  ...
  const isLoadingRetweetsRef = useRef(false)
  const loadProfileAbortController = useRef<AbortController | null>(null)

  const loadCreatorProfile = useCallback(async () => {
    if (!username) return

    if (loadProfileAbortController.current) {
      loadProfileAbortController.current.abort()
    }
    loadProfileAbortController.current = new AbortController()
    ...
    try {
      ...
      if (creatorProfile && creatorProfile.userType === "creator") {
        ...
        const [creatorPosts, creatorHighlights, creatorServices, stories] = await Promise.all([
          getPostsByAuthor(creatorProfile.username),
          getCreatorHighlights(creatorProfile.uid),
          getCreatorServices(creatorProfile.uid),
          (async () => {
            const { getCreatorActiveStories } = await import("@/lib/firebase/firestore")
            return getCreatorActiveStories(creatorProfile.uid)
          })(),
        ])
        ...
        if (user) {
          const { hasUnviewedStories } = await import("@/lib/firebase/firestore")
      ...
      if (user) {
        const [currentProfile, level] = await Promise.all([getUserProfile(user.uid), getCurrentUserLevel(user.uid)])
  ...
  }, [username, user, loadCreatorProfile]) // Added loadCreatorProfile to dependencies

  useEffect(() => {
    const checkLikedPosts = async () => {
      if (!user || displayedPosts.length === 0 || isLoadingLikesRef.current) return

      isLoadingLikesRef.current = true
      try {
        const likeChecks = await Promise.all(
          displayedPosts.map(async (post) => {
            if (!post.id) return { postId: null, isLiked: false }
  ...
  }, [user, displayedPosts.length])

  useEffect(() => {
    const checkRetweetedPosts = async () => {
      if (!user || displayedPosts.length === 0 || isLoadingRetweetsRef.current) return

      isLoadingRetweetsRef.current = true
      try {
        const retweetChecks = await Promise.all(
          displayedPosts.map(async (post) => {
            if (!post.id) return { postId: null, isRetweeted: false }
            const isRetweeted = await checkUserRetweeted(user.uid, post.id)
            return { postId: post.id, isRetweeted }
          }),
        )
        ...
        retweetChecks.forEach(({ postId, isRetweeted }) => {
          if (postId && isRetweeted) {
            retweetedSet.add(postId)
          }
        })

        // Removed setRetweetedPosts state
        // setRetweetedPosts(retweetedSet)
      } catch (error) {
        console.error("Error checking retweeted posts:", error)
      } finally {
        isLoadingRetweetsRef.current = false
      }
    }
  ...
  const handleShare = async (postId: string) => {
    ...
    try {
      const result = await handleRetweetToggle(postId, creator.uid)
  ...
  const hasContentAccess = useCallback(
    ...
    [isOwner, userSubscriptions, creator],
  ...
  const handleViewStoryFromMenu = useCallback(() => {
  ...
  const handleHighlightClick = useCallback(
    (highlight: CreatorHighlight, event?: React.MouseEvent) => {
      if (event) {
    ...
    [hasContentAccess, setPaywallToast],
  )

  // The original openStoryViewer now calls the new handler
  const openStoryViewer = (highlight: CreatorHighlight, event?: React.MouseEvent) => {
    handleHighlightClick(highlight, event)
  }

  // The original closeStoryViewer now calls the new handler
  const closeStoryViewer = () => {
    handleStoryViewerClose()
  }

  // The original openTemporaryStoriesViewer now calls the new handler
  const openTemporaryStoriesViewer = (event?: React.MouseEvent) => {
    ...
    if (activeStories.length > 0) {
      handleViewStoryFromMenu()
    }
  }

  const handleProfilePictureClick = (event?: React.MouseEvent) => {
    if (event) {
      event.preventDefault()
      event.stopPropagation()
    }

    // If has stories, show menu with options
    if (activeStories.length > 0) {
      setShowProfileMenu(true)
    } else {
      // If no stories, just show profile picture
      setShowProfilePictureViewer(true)
    }
  }

  const handleViewProfilePictureFromMenu = () => {
    setShowProfilePictureViewer(true)
  }

  const mappedTemporaryStories = useMemo(() => {
    if (!viewingTemporaryStories || activeStories.length === 0) {
      return []
    }

    const mapped = activeStories.map((story) => {
      return {
        id: story.id,
        imageUrl: story.mediaType === "image" ? story.mediaUrl : undefined,
        videoUrl: story.mediaType === "video" ? story.mediaUrl : undefined,
        isTemporary: true,
        createdAt: story.createdAt,
        expiresAt: story.expiresAt,
        creatorId: creator?.uid || "",
        duration: 24, // Assuming 24 hours for temporary stories
      }
    })

    return mapped
  }, [viewingTemporaryStories, activeStories, creator?.uid])

  const mappedHighlightStories = useMemo(() => {
  ...
  }, [selectedHighlight, creator?.uid])

  const storiesToShow = useMemo(() => {
    const stories = viewingTemporaryStories ? mappedTemporaryStories : selectedHighlight ? mappedHighlightStories : []
    return stories
  }, [viewingTemporaryStories, selectedHighlight, mappedTemporaryStories, mappedHighlightStories])
    ...
    <ResponsiveLayout showSuggestions={true}>
      <div className="min-h-screen bg-background">
                ...
                {isOwner ? (
                  ...
                  >
                    <StoryRingAvatar
                      ...
                      isOwner={isOwner}
                      size="xl"
                      onClick={(e) => handleProfilePictureClick(e as any)}
                    />
                  </StoryCreatorButton>
                ) : (
                  <StoryRingAvatar
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-around py-3 sm:py-4 bg-card rounded-xl border border-border shadow-sm">
                <div className="text-center">
                  <div className="text-lg sm:text-xl font-bold text-primary">{allPosts.length}</div>{" "}
                  {/* Use allPosts */}
                ...
                {displayedPosts.length === 0 ? ( // Use displayedPosts
                  <div className="text-center py-12 space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Os posts de {creator.displayName} aparecerão aqui
                    </div>
                  </div>
                ) : (
                            ...
                            <div className="relative">
                              <div className={!hasContentAccess(post.requiredLevel) ? "filter blur-md" : ""}>
                                ...
                                {post.images && post.images.length > 0 && (
                                  <div className="mb-4 rounded-lg overflow-hidden relative aspect-square">
                                    <Image
          ...
          <StoryViewer
            ...
            stories={
              selectedCreatorStories
                ? selectedCreatorStories.stories
                : selectedHighlight
                  ? [
                      {
                        id: selectedHighlight.id,
                  ...
                  : viewingTemporaryStories
                    ? mappedTemporaryStories
                    : [] // Use mappedTemporaryStories if viewingTemporaryStories is true
            }
            initialStoryIndex={currentStoryIndex} // Pass currentStoryIndex
            creatorName={selectedCreatorStories ? selectedCreatorStories.creatorName : creator?.displayName || ""}
            creatorUsername={selectedCreatorStories ? selectedCreatorStories.creatorUsername : creator?.username || ""}
            creatorAvatar={
              selectedCreatorStories
                ? selectedCreatorStories.creatorAvatar
                : creator?.profileImage || "/placeholder.svg"
            }
            ...
            onStoryViewed={(storyId) => {
              // Handle viewed story logic if needed, e.g., marking as viewed for 'hasUnviewed'
            }}
          />
        )}
        ...
        <TipModal
          ...
          postId={selectedTipPost?.id || ""} // Ensure postId is not null
          currentUserId={user?.uid || ""} // Ensure currentUserId is not null
          userSubscriptions={currentUserProfile?.subscriptions || []} // Ensure subscriptions is not null
        />
        ...
        <ProfilePictureMenu
          ...
          onViewStory={handleViewStoryFromMenu}
          onViewProfilePicture={handleViewProfilePictureFromMenu}
