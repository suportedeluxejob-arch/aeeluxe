  ...
  const loadMLMData = async () => {
    try {
      ...
      const commissionsData = commissionsSnapshot.docs
        ...
        .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()) as CommissionTransaction[]

      setCommissions(commissionsData)

      // Load platform revenue transactions
      const platformRef = collection(db, "transactions")
      const platformQuery = query(platformRef, where("creatorId", "==", "PLATFORM"), limit(1000))
      ...
      const networkQuery = query(networkRef, limit(1000))
