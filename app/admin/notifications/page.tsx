...
import { collection, addDoc, serverTimestamp, getDocs, query, where, doc, updateDoc, limit } from "firebase/firestore"
...
export default function NotificationsPage() {
  ...
  async function loadTemplates() {
    try {
      ...
      const q = query(templatesRef, where("isAutomatic", "==", true), limit(50))
  ...
  async function handleSendNotification() {
    ...
    try {
      ...
      if (target === "all_users") {
        ...
        const q = query(usersRef, where("userType", "!=", "creator"), limit(5000))
        const snapshot = await getDocs(q)
        ...
        const usersRef = collection(db, "users")
        const q = query(usersRef, where("level", "==", specificTier), limit(5000))
        const snapshot = await getDocs(q)
        targetUserIds = snapshot.docs.map((doc) => doc.id)
      }
