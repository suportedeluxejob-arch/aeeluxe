...
import { validateAndCropImage } from "@/lib/image-utils"

import { Button } from "@/components/ui/button"
import { Textarea } from "@/components/ui/textarea"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { X, ImageIcon, Loader2, VideoIcon, Sparkles, Upload, Trash2 } from "lucide-react"
...
export default function CreatePostModal({ onClose }: CreatePostModalProps) {
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const processedImages = await Promise.all(
      files.map(async (file) => {
        try {
          const croppedBlob = await validateAndCropImage(file)
    ...
    const validImages = processedImages.filter((img) => img !== null) as { file: File; preview: string }[]
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "posts"), {
        ...
        requiredLevel,
        authorDisplayName: userProfile?.displayName || "Creator",
        authorUsername: userProfile?.username || "creator",
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
        <div className="bg-card w-full max-w-2xl rounded-xl border border-border max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between p-4 border-b border-border sticky top-0 bg-card/95 backdrop-blur-sm z-10">
            <h2 className="text-lg font-semibold">Criar Conteúdo</h2>
          ...
          {/* Tabs */}
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "post" | "story")} className="w-full">
            <TabsList className="w-full grid grid-cols-2 mx-4 my-4" style={{ width: "calc(100% - 2rem)" }}>
            ...
            <TabsContent value="post" className="p-4 space-y-4 mt-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-primary">{formatNumber(likes)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        className="h-12 px-3 flex-1 bg-background/50 text-sm font-medium"
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-primary">{formatNumber(comments)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
              ...
              <div className="space-y-2">
                ...
                <Select value={requiredLevel} onValueChange={(value: any) => setRequiredLevel(value)}>
                  ...
                  <SelectContent>
                    <SelectItem value="Bronze">Bronze (Gratuito)</SelectItem>
            ...
            <TabsContent value="story" className="p-4 mt-0">
              <div className="text-center py-8 space-y-4">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-primary/20 to-pink-500/20 mb-2">
                  <Sparkles className="h-8 w-8 text-primary" />
                </div>
                <div>
                  <h3 className="font-bold text-lg mb-2">Criar Story ou Destaque</h3>
                  <p className="text-sm text-muted-foreground mb-6">
                    Publique stories temporários (24h) ou crie destaques permanentes no seu perfil
                  </p>
                </div>
                <Button
                ...
                >
                  ...
                  Abrir Editor de Stories
                </Button>
              </div>
            </TabsContent>
          </Tabs>

          {/* Footer - Only for Post Tab */}
          {activeTab === "post" && (
            <div className="flex items-center justify-end space-x-2 p-4 border-t border-border sticky bottom-0 bg-card/95 backdrop-blur-sm">
      ...
      {/* Story Creator Modal */}
      {showStoryModal && user && userProfile && (
        <StoryCreatorModal
